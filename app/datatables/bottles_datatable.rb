class BottlesDatatable
  delegate :params, :h, :link_to, :edit_bottle_path, :bottle_path, :coa_summary_path, :button_to, :image_tag, to: :@view
...
private
  ...
  def data
    ...
    bottles.map do |bottle|
      [
          ...
          "<a href= #{coa_summary_path(bottle.coa_summary_id)} data-remote='true'>" \
          "<button class='btn btn-default btn-xs'>"+ "#{bottle.coa_summary_id} "\
          "<span class='fa fa-list-ul'></span></button></a>",
          ...
          ERB::Util.h(human_boolean(bottle.can_plate)),
          ERB::Util.h(bottle.comment),
          "<a href= #{edit_bottle_path(bottle.id)} data-remote='true'>" \
          "<button class='btn btn-primary btn-xs'> Edit <span class='fa fa-pencil-square-o'></span></button></a>"
      ]
  ...
  def sort_column
    columns = %w[barcode_parent barcode compound_name cas qty_available_mg_ul units vendor sam cpd coa_summary_id can_plate comment]
